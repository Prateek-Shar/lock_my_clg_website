...
fetch("/get_overall_rating_iim_kozhikode")
    .then(response => {
        if (!response.ok) {
            if (response.status === 404) {
                console.log("No rating data available");
                return { rating: "0.00" }; // Return default rating for empty data
            }
    ...
    .then(data => {
        ...
        if (isNaN(rating)) {
            ...
            rating = 0; // Set default rating to 0 for invalid data
        }
    ...
    .catch(error => {
        console.error("Error fetching rating:", error);
        // Set default rating to 0 on error
        colorStars(0);
        updateRatingText(0);
    });
...
async function fetchRating6() {
    try {
        const response = await fetch("/get_placement_rating_iim_kozhikode");
        if (!response.ok) {
            if (response.status === 404) {
                console.log("No placement rating data available");
                displayRating(0);
                return;
            }
            throw new Error(`HTTP error! status: ${response.status}`);
        ...
        if (isNaN(rating)) {
            console.error("Error: Invalid rating value received");
            displayRating(0);
            return;
        }
...
function displayRating(rating) {
    ...
    for (let i = 1; i <= totalStars; i++) {
        const star = document.createElement("span");
        if (i <= Math.floor(rating)) {
            star.innerHTML = `<img src="/images/filled_star.png" alt="Full Star" width="20" height="20">`;
            star.classList.add("gold");
        } else if (rating - i + 1 >= 0.5) {
            star.innerHTML = `<img src="/images/half_star.png" alt="Half Star" width="20" height="20">`;
            star.classList.add("half");
        } else {
            star.innerHTML = `<img src="/images/star.png" alt="Empty Star" width="20" height="20">`;
...
async function fetchRating7() {
    try {
        const response = await fetch("/get_campus_life_rating_iim_kozhikode");
        const data = await response.json();
        const rating = data.rating; 
...
async function fetchRating8() {
    try {
        const response = await fetch("/get_faculty_rating_iim_kozhikode");
        const data = await response.json();
        const rating = data.rating; 
...
async function fetchRating9() {
    try {
        const response = await fetch("/get_infrastructure_rating_iim_kozhikode");
        const data = await response.json();
        const rating = data.rating; 
...
async function fetchRating10() {
    try {
        const response = await fetch("/get_curriculum_rating_iim_kozhikode");
        const data = await response.json();
        const rating = data.rating; 
